...
7. **각 불릿은 간결한 명사구로 약 40자 이내를 권장합니다.** 도구의 실제 구성 요소(웹훅·노드·라우터·Composer 등)는 반영하되, 워크플로우 단계는 화살표(→)·가운뎃점(·)으로 압축하고 장황한 서술 문장은 피합니다. 실제 제안서 커리큘럼표는 짧은 명사구 나열이 표준 톤입니다 — 도구를 구체화하되 길이는 원본 수준을 넘기지 마세요
...
[좋은 예 — Make로 전환] (간결한 명사구, ~40자)
원본: "[실습] IDE형 도구로 간단한 웹페이지 제작 (계산기·구구단·날씨 알리미)"
좋음: "[실습] Make 시나리오 — 웹훅 → Sheets 읽기 → 라우터 분기 → Slack 전송"

[좋은 예 — LangGraph로 전환]
원본: "- [실습] 간단한 챗봇 구현 (API 연동, Rule 작성)"
좋음: "- [실습] LangGraph StateGraph 챗봇 — 노드·조건부 엣지 구성, interrupt로 사람 승인"

[좋은 예 — Cursor로 전환]
원본: "- 핵심 워크플로우: 프롬프트 입력 → 코드 자동 생성 → 실행 승인"
좋음: "- 핵심 워크플로우: Cursor Composer(Cmd+I) → @파일 컨텍스트 지시 → 멀티파일 편집 검토·승인"

[좋은 예 — n8n으로 전환]
원본: "- [실습] 엑셀 파일 자동 합치기 + 요약 리포트 생성"
좋음: "- [실습] n8n 워크플로우 — Schedule → Drive 엑셀 읽기 → 병합 → Code 노드 집계 → Slack 전송"

[좋은 예 — Claude Code로 전환]
원본: "- [실습] 자연어 지시로 간단한 도구 제작 (구구단·계산기)"
좋음: "- [실습] Claude Code 터미널 — 자연어 지시로 파일 생성(Write/Edit) → Bash 실행·테스트 → 수정 반복"

[나쁜 예 — Tool 이름만 접두]
나쁨: "Make 코드 자동 생성·Make 웹 앱 배포·Make 디버깅"
좋음: 위 Make 예시처럼 실제 구성 요소(시나리오·라우터·웹훅)를 화살표로 압축

[나쁜 예 — 장황한 서술]
나쁨: "[실습] Make 시나리오 구성 — 웹훅 트리거 → Google Sheets 데이터 읽기 → 라우터로 조건 분기 → Slack 메시지 전송, 에러 핸들러로 재시도 로직 추가" (60자+ 장황)
좋음: 위 Make 예시처럼 핵심 단계만 화살표로 압축 (~40자)
...
- 불릿이 약 50자를 크게 초과하는 장황한 서술 (도구 단계를 화살표→·가운뎃점·로 압축할 것)
...
const APP_VERSION = 'rewrite-by-tool-v2-concise';
